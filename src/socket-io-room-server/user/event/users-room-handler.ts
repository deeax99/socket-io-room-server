...
export default class UsersRoomHandler {
    ...
    handleUser = (userConnection: ServerUser) => {
        ...
        socket.on("joinRoom", (name, callback) => {
            const result = this.roomsHandler.joinRoom(userConnection, name, callback);
            callback(result);
        });

        socket.on("leaveRoom", (callback) => {
            const result = this.roomsHandler.leaveRoom(userConnection, callback);
            callback(result);
        });

        socket.on("createRoom", (name, callback) => {
            const result = this.roomsHandler.createRoom(userConnection, name, callback);
            callback(result);
        });
    }
} 