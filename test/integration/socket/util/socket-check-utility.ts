import {KeyValueData} from '../../../../src/socket-io-room-server/dto/data-key-value-change';
...
export class SocketCheckUtility {
  ...
  checkRoomClients(roomName: string, clientConnections: ClientUserConnection[]) {
    ...
    const lhs = clientConnections.map((clientConnection) => clientConnection['clientUserConnectionSocket'].socket.id);
  ...
  private arraysEqual(a: any[], b: any[], withOrder = true): boolean {
    if (a === b) {
      return true;
    }
    if (a == null || b == null) {
